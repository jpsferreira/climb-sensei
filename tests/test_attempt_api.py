...
class TestListAttempts:
    def test_list_attempts_empty(self, client, sample_route):
        ...
        response = client.get(f"/api/v1/routes/{sample_route.id}/attempts")
    ...
    def test_list_attempts_other_user_route(
        ...
        response = client.get(f"/api/v1/routes/{other_route.id}/attempts")
        assert response.status_code == 404
...
class TestGetAttemptDetail:
    def test_get_attempt_detail_success(
        ...
        response = client.get(
            f"/api/v1/routes/{sample_route.id}/attempts/{sample_attempt.id}"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_attempt_detail_with_deltas(
        ...
        response = client.get(
            f"/api/v1/routes/{sample_route.id}/attempts/{attempt2.id}"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_attempt_not_found(self, client, sample_route):
        ...
        response = client.get(f"/api/v1/routes/{sample_route.id}/attempts/99999")
        assert response.status_code == 404

    def test_get_attempt_route_not_found(self, client, sample_attempt):
        """Should return 404 when route doesn't exist."""
        response = client.get(f"/api/v1/routes/99999/attempts/{sample_attempt.id}")
        assert response.status_code == 404
...
class TestUpdateAttempt:
    ...
    def test_update_attempt_not_found(self, client, sample_route):
        """Should return 404 for non-existent attempt."""
        response = client.patch(
            f"/api/v1/routes/{sample_route.id}/attempts/99999",
            json={"notes": "ghost"},
        )
        assert response.status_code == 404
    ...
    def test_update_attempt_route_not_found(self, client, sample_attempt):
        """Should return 404 when route doesn't exist."""
        response = client.patch(
            f"/api/v1/routes/99999/attempts/{sample_attempt.id}",
            json={"notes": "ghost"},
        )
        assert response.status_code == 404
...
class TestDeleteAttempt:
    ...
    def test_delete_attempt_not_found(self, client, sample_route):
        """Should return 404 for non-existent attempt."""
        response = client.delete(f"/api/v1/routes/{sample_route.id}/attempts/99999")
        assert response.status_code == 404

    def test_delete_attempt_route_not_found(self, client, sample_attempt):
        """Should return 404 when route doesn't exist."""
        response = client.delete(f"/api/v1/routes/99999/attempts/{sample_attempt.id}")
        assert response.status_code == 404
...
class TestMetricTrend:
    def test_get_metric_trend_empty(self, client, sample_route):
        ...
        response = client.get(f"/api/v1/routes/{sample_route.id}/progress/avg_velocity")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_metric_trend_invalid_metric(self, client, sample_route):
        ...
        response = client.get(f"/api/v1/routes/{sample_route.id}/progress/hacked_field")
    ...
    def test_get_metric_trend_all_allowed_metrics(self, client, sample_route):
        ...
        for metric in ALLOWED_METRICS:
            response = client.get(f"/api/v1/routes/{sample_route.id}/progress/{metric}")
